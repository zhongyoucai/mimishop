...
class Goods extends base
{
	...
	public function index(){
		...
		foreach($parentCate as $key=>$val){
			$parentCate[$key]['selfCate'] = Db::name('GoodsCate')->where(['pid'=>$val['id']])->order('id asc')->select();
		}
    ...
    public function detail(){
		...
		//购物车数量
		$cart = Db::name('Cart')->where(['uid'=>session('wap_user_auth.uid'),'status'=>1])->field("SUM(num) as num")->find();
		$cart_count = !empty($cart['num']) ? $cart['num'] : 0;
		...
		//获取评价数量
		$comment_count = Db::name('GoodsComment')->where($map)->count();
		
		$gdata['goods_id'] = $id;
		$gdata['uid'] = session('wap_user_auth.uid');
		//检查商品是否收藏
		$collection = Db::name('GoodsCollection')->where($gdata)->count();
    ...
    public function collection(){
        ...
        if($info){
            if(Db::name('GoodsCollection')->where(['id'=>$info['id']])->delete()){
                return $this->success('取消收藏成功','',['status'=>0]);
            ...
            if(Db::name('GoodsCollection')->insert(['uid'=>$uid,'goods_id'=>$id,'createtime'=>time()])){
                return $this->success('收藏成功','',['status'=>1]);
